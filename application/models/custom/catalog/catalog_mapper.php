...
class Catalog_mapper extends MY_Model implements Mapper
{
  ...
  public function get_category_list ( $section_id = 0, $order_title = 'id', $order_type = 'DESC' )
  {
    ...
    public function get_category_list2 ( $section_id = 0, $order_title = 'id', $order_type = 'DESC' )
  {
    $this->db->select('*');
    $this->db->from(self::TABLE_CATALOG_CATEGORY);
    $this->db->where('parent_category_id', $section_id);
    $this->db->where('is_deleted !=', 1);
    $this->db->order_by($order_title, $order_type);
    return $this->db->get()->result(self::CLASS_CATEGORY);
  }
  ...
  public function get_category_item_list ( $category_id = 0, $order_title = 'id', $order_type = 'DESC' )
  {
    ...
    public function get_category_item_list100 ( $category_id = 0, $order_title = 'id', $order_type = 'DESC' )
  {
    $this->db->distinct();
    $this->db->select('
      catalog_item.*
    ...
    $this->db->where('catalog_category.is_deleted', 0);
    $this->db->where('catalog_item.is_deleted', 0);
    $this->db->where('catalog_item.in_stock', 1);
    $this->db->where("(`catalog_category`.`id` = '{$category_id}' OR `catalog_category`.`parent_category_id` = '{$category_id}')");
    $this->db->order_by($order_title, $order_type);
    $tmp = $this->db->get()->result(self::CLASS_ITEM);
    foreach ($tmp as $key => $tmp_item) {
  ...
  public function get_page_content ( $page_id = 0 )
  {
    ...
    // add to cart
    if ( isset($_REQUEST['type']) && ($_REQUEST['type'] == 'add') && isset($_REQUEST['item_id']) && isset($_REQUEST['qty']) )
    {
    ...
    if ( ! empty($section) )
    {
      ...
      if ( $item_id = $this->input->get('item') )
      {
        if ( $section->similar_items )
        {
          ...
          if ( !empty($item_cats) )
          {
            ...
            {
              $similar_items = array_merge($similar_items, $this->get_category_item_list100($category->id));
            }
      ...
      {
        $catalog_products = $this->get_category_item_list($cat_id, 'in_stock');
        if ( isset($_REQUEST['type']) && ($_REQUEST['type'] == 'filtr'))
        { 
                ...
                $this->db->select('*');
                $this->db->from('catalog_category');
                $this->db->where('parent_category_id', $cat_id);
                $results = $this->db->get()->result(self::CLASS_CATEGORY);
                $index=0;
                if(!empty($results)){
                    
                    foreach($results as $ind => $resulti)
                    {
                        $this->db->select('*');
                        $this->db->from('catalog_category');
                        $this->db->where('parent_category_id', $resulti->id);
                        $result = $this->db->get()->result(self::CLASS_CATEGORY);
                        if(!empty($result)){
                            ...
                            foreach($result as $key=>$res){
                                ...
                                $this->db->select('*');
                                $this->db->from('catalog_category');
                                $this->db->where('parent_category_id', $res->id);
                                $result5 = $this->db->get()->result(self::CLASS_CATEGORY);
                                if(!empty($result5)){
                                    
                                    foreach($result5 as $key5=>$res5){
                                    
                                        $this->db->select('*');
                                        $this->db->from('catalog_item');
                                        $this->db->join('catalog_item_links', 'catalog_item_links.item_id = catalog_item.id');
                                        $this->db->where('catalog_item_links.category_id', $res5->id);
                                        $this->db->where('catalog_item.is_deleted', 0);
                                        if(!empty($_REQUEST['material'])){
                                        $this->db->where('catalog_item.material', $_REQUEST['material']);
                                        }
                                        if(!empty($_REQUEST['brands'])){
                                        $this->db->where('catalog_item.brand', $_REQUEST['brands']); 
                                        }
                                        if(!empty($_REQUEST['price'])){                                
                                            if($_REQUEST['price'] == 1){
                                                $this->db->order_by('catalog_item.price', 'asc');
                                            }
                                            if($_REQUEST['price'] == 2){
                                                $this->db->order_by('catalog_item.price', 'desc');
                                            }
                                        }else{
                                            $this->db->order_by('catalog_item.in_stock', 'desc');
                                        }
                                        $catalog_pr = $this->db->get()->result(self::CLASS_ITEM); 
                                        $index2=$index;
                                        if($ind == 0){
                                            foreach($catalog_pr as $index=>$catalog_p){
                                                $catalog_prod[$index] = $catalog_p;
                                            }
                                        }else{    
                                            foreach($catalog_pr as $index=>$catalog_p){
                                                $catalog_prod[$index+$index2+1] = $catalog_p;
                                            }
                                            $index=$index+$index2;
                                        }
                                    }
                                    $this->db->select('*');
                                        $this->db->from('catalog_item');
                                        $this->db->join('catalog_item_links', 'catalog_item_links.item_id = catalog_item.id');
                                        $this->db->where('catalog_item_links.category_id', $res->id);
                                        $this->db->where('catalog_item.is_deleted', 0);
                                        if(!empty($_REQUEST['material'])){
                                        $this->db->where('catalog_item.material', $_REQUEST['material']);
                                        }
                            ...
                            $this->db->from('catalog_item');
                            $this->db->join('catalog_item_links', 'catalog_item_links.item_id = catalog_item.id');
                            $this->db->where('catalog_item_links.category_id', $resulti->id);
                            $this->db->where('catalog_item.is_deleted', 0);
                                if(!empty($_REQUEST['material'])){
                                $this->db->where('catalog_item.material', $_REQUEST['material']);
                                }
                    ...
                    $this->db->from('catalog_item');
                    $this->db->join('catalog_item_links', 'catalog_item_links.item_id = catalog_item.id');
                    $this->db->where('catalog_item_links.category_id', $cat_id);
                    $this->db->where('catalog_item.is_deleted', 0);
                        if(!empty($_REQUEST['material'])){
                        $this->db->where('catalog_item.material', $_REQUEST['material']);
                        }
            ...
            if((!empty($_REQUEST['colour'])) && (!empty($_REQUEST['size']))){
            $indexgood=0;
                foreach($catalog_prod as $indexprod => $cat_prod){
                $flaq1=0;
                $flaq2=0;
                    if(!empty($cat_prod->colors)){                    
                        $str2 = str_replace('[','', $cat_prod->colors);
                        $str2 = str_replace(']','', $str2);
                        $str = explode(';', $str2);  
                        foreach($str as $key=>$st){
                        }
                        foreach($str as $key2=>$st){
                            if($key != $key2){
                            $st = str_replace(';','', $st);
                                if((int)$st == (int)$_REQUEST['colour']){
                                    $flaq1=1;
                                }
                    ...
                    if(!empty($cat_prod->size)){
                    
                        $str2 = str_replace('[','', $cat_prod->size);
                        $str2 = str_replace(']','', $str2);
                        $str = explode(';', $str2); 
                        foreach($str as $key=>$st){
                        }
                    ...
                    if(($flaq2 == 1) && ($flaq2 == 1)){
                        $catalog_products[$indexgood]=$cat_prod; 
                        $indexgood++;  
                    }
                }
            }else if (!empty($_REQUEST['colour'])){
            $indexgood=0;
                foreach($catalog_prod as $indexprod => $cat_prod){
                $flaq1=0;
                    if(!empty($cat_prod->colors)){
      ...
      } else if ( $met = $this->input->get('met') ){
            if ( $met == 1 ){
                $catalog_products = $this->get_new_product();
                if ( isset($_REQUEST['type']) && ($_REQUEST['type'] == 'filtr')){ 
                    unset($catalog_products);
                    $catalog_products=array();
                    $catalog_prod=array();
                    ...
                    $this->db->from('catalog_item');
                                ...
                                $this->db->where('is_deleted', 0);
                                if(!empty($_REQUEST['material'])){
                                $this->db->where('material', $_REQUEST['material']);
                                }
                                if(!empty($_REQUEST['brands'])){
                                $this->db->where('brand', $_REQUEST['brands']); 
                                }
                            if(!empty($_REQUEST['price'])){                                
                                if($_REQUEST['price'] == 1){
          ...
          if ( $met == 2 ){
                $catalog_products = $this->get_sale_product();
                if ( isset($_REQUEST['type']) && ($_REQUEST['type'] == 'filtr')){ 
                    unset($catalog_products);
                    $catalog_products=array();
                    $catalog_prod=array();
          ...
          if ( $met == 3 ){
              //$catalog_products = $this->get_category_item_list(21);
              $catalog_products = $this->get_category_item_list(29, 'in_stock');
              $catalog_products2 = $this->get_category_item_list(2, 'in_stock');
              $catalog_products3 = $this->get_category_item_list(122, 'in_stock');
              $catalog_products4 = $this->get_category_item_list(21, 'in_stock');
              $index1=0;
              $index2=0;
              $index3=0;
              $index4=0;
              foreach($catalog_products as $index1=>$cat_pr1){              
              }
              foreach($catalog_products2 as $index2=>$cat_pr2){
                $catalog_products[$index2+$index1+1]=$cat_pr2;
              }
              foreach($catalog_products3 as $index3=>$cat_pr3){
                $catalog_products[$index3+$index2+$index1+1]=$cat_pr3;
              }
              foreach($catalog_products4 as $index4=>$cat_pr4){
                $catalog_products[$index4+$index3+$index2+$index1+1]=$cat_pr4;
              }
              if ( isset($_REQUEST['type']) && ($_REQUEST['type'] == 'filtr')){  
                unset($catalog_products);
                $catalog_products=array();
                $catalog_prod=array();
                $result=$this->get_met(4, 21, 29, 2, 122 );
                $index2=0;
                $index=0;
                ...
                foreach($result as $key=>$res){
                    ...
                    $this->db->from('catalog_category');
                    $this->db->where('parent_category_id', $res->id);
                    $resultsi= $this->db->get()->result(self::CLASS_CATEGORY);
                    if(!empty($resultsi)){
                        foreach($resultsi as $key2 => $res2){
                            $this->db->select('*');
                            $this->db->from('catalog_item');
                            $this->db->join('catalog_item_links', 'catalog_item_links.item_id = catalog_item.id');
                            $this->db->where('catalog_item_links.category_id', $res2->id);
                            $this->db->where('catalog_item.is_deleted', 0);
                            if(!empty($_REQUEST['material'])){
                            $this->db->where('catalog_item.material', $_REQUEST['material']);
                            }
          ...
          if ( $met == 4 ){
              $catalog_products = $this->get_category_item_list(30, 'in_stock');
              $catalog_products2 = $this->get_category_item_list(31, 'in_stock');
              $index1=0;
              foreach($catalog_products as $index1=>$cat_pr1){              
              }
              foreach($catalog_products2 as $index2=>$cat_pr2){
      ...
      }else if ( isset($_REQUEST['type']) && ($_REQUEST['type'] == 'filtr')){  
        unset($catalog_products);
         $catalog_products=array();
         $catalog_prod=array();
                                        $this->db->select('*');
      ...
      }else if ( $brand = $this->input->get('brand') )
      {
          $catalog_products = $this->get_brands($brand);       
      }else {
        $catalog_products = $this->get_section_item_list($section->id, 'in_stock');
      }
...
================================================================================*/
  ...
  protected function _get_pages_tree_up ( $page_id, $pages_array = array() )
  {
    ...
    public function get_met ($num = 4, $id1= 29, $id2 = 2, $id3= 122, $id4= 21)
    {
        $result=array();
        for($i=0;$i < $num; $i++){
            if($i==0){
                ...
                $res[$i]=$this->db->get()->result(self::CLASS_CATEGORY);
            }
            if($i==1){
                $this->db->select('*');
                $this->db->from(self::TABLE_CATALOG_CATEGORY);
  ...
  /*-- page breadcrumbs end --------------------------------------------------*/
    public function get_parent_category ($id, $order_type='id' )
    {
        $this->db->select('*');
        $this->db->from(self::TABLE_CATALOG_CATEGORY);
        $this->db->where('is_deleted !=', 1);
        $this->db->where($order_type, $id);
